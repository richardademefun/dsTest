...
from desc import fixed_head, overview_desc,code, code2, overview_desc, home_page, fixed_head, code3, code4, code5
...
def page1():
    ...
    image = Image.open(initial_df.resolve())
    ...
    One dataset to Do the CPA anlysis and the Click Through rate. You can see this as when ever pixel id != 0 
    there are no ctr related metrics for example.""")

    st.code(code2, language='python')
    image = Image.open(common_ids)
    st.image(image, caption='commonn ids')
    st.write('## CPA Analysis')

    st.write("""Since I was given th choice as to which Line_item_id i get to analysed. I have chosen the
    one with the highest occourences of our target KPI's(Target Registration CPA: $1,000 (Pixel ID: 1433481) &
    Target Deposit CPA: $1,400 (Pixel ID: 1433583))""")

    st.code(code3, language='python')
    st.code(code4, language='python')

    st.write("Drop all irrelevent columns. And create classifing column for both desired CPA value of 1433583 and 1433481")

    st.code(code5, language='python')

    st.write("""Aggrigate the model to the model leaf to evaluate how well models are performing on average. 
    Moreover we need to also know what a good value for form completion an first deposit value would be. I found a link 
    to an article explaining that the average sign up rate is [30%](https://baymard.com/lists/cart-abandonment-rate). 
    Unfortunately I couldn't find anything similar for first deposit rates so i made the assumption that 1/3 of 
    people who complete the sign up also make a depsit giveing me an average rate of 10% for the Target Deposit CPA value.""")
    ...
    st.write("""For the ctr analysis we went through a similare process as above where we filtered the 
    data to keep all the fields and rows relevent to the ctr. Moreover when checking for the frequency
    ...
    st.write("""For Ctr i also aggrigated it usine the mean. i also tryed a using a weighted mean aggriation 
